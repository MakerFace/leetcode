...
private:
    // TODO 控制回退
    bool find_next(vector<vector<node>> &graph, char c, int &row, int &col)
    {
        int i = row, j = col;
    ...
    bool find_head(vector<vector<node>> graph, char c, int &row, int &col)
    {
        ++col;
...
{
    Solution s;
    vector<vector<char>> board = {{'Z','Z','Z','Z','Z'},{'Z','C','B','A','B'},{'A','D','E','D','C'},{'B','E','Z','Z','Z'}};
