...
class Solution {
 public:
  vector<string> restoreIpAddresses(string s) {
    ...
    for (size_t l = 1; l < 4 && s.size(); ++l) {
      for (size_t m = l + 1; m < l + 4 && m < s.size(); ++m) {
        for (size_t r = m + 1; r < m + 4 && r < s.size(); ++r) {
          string temp[4];
          temp[0] = s.substr(0, l);
 ...
 private:
  bool isValidAddress(string &s) {
    ...
    if (s[0] >= '3' && s.size() == 3) return false;
